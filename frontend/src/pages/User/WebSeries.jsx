...
const WebSeries = () => {
    ...
    document.title = APPNAME+'WEB SERIES'
    const [loader, setLoader] = useState(false)
    const [categories, setCategories] = useState([])
    const [webSeries, setWebSeries] = useState([])
    const getSportsDetailsList = async() =>{
        setLoader(true)
        await AxiosClient.get(`/user_web_series_details/${'web-series'}`)
        .then(({data})=>{
            setLoader(false)
                        ...
                        <div className='my-4 md:my-4 grid grid-cols-3 md:grid-cols-6 gap-2 md:gap-4'>
                            {categories.map((data, id)=>{
                                return(<Fragment key={id}>
                                    <Link to={'/web-series/'+data.sub_category_slug} className='bg-white border border-gray-300 py-3 md:py-4 rounded-lg shadow-md hover:bg-primaryColor hover:border-none cursor-pointer hover:text-white uppercase font-bold tracking-widest text-xs md:text-base text-center flex items-center'>
